...
/**
 * PHP一句话扫描脚本加强版，可扫描 weevelyshell 生成 或加密的shell。
 */
define("SELF", phpSelf());
...
function antivirus($dir, $exs, $matches)
{
    ...
    while (false !== ($name = readdir($handle))) {
        ...
        if (is_dir($path)) {
            ...
            echo '特征 <input type="text" style="width:250px;" value="解析漏洞"> 	' . $path . '<div></div>';
            flush();
            ob_flush();
        } else {
            if (!preg_match($exs, $name)) {
                continue;
            }
            ...
            if (weevelyshell($path)) {
                echo '特征 <input type="text" style="width:250px;" value="weevely 加密shell"> 	' . $path . '<div></div>';
                flush();
                ob_flush();

            } elseif (callbackshell($path)) {
                echo '特征 <input type="text" style="width:250px;" value="Callback shell"> 	' . $path . '<div></div>';
                flush();
                ob_flush();
            }
