...
class downloader
{
     ...
     * @throws Exception
     */
    public static function get($url, $save_file, $speed = 10240, $headers = array(), $timeout = 10)
    {
        ...
        // get content length
        $content_length = self::getContentSize($url_info['host'], $url_info['port'], $url_info['request'], $headers, $timeout);
        ...
        try {
            $download_status = self::downloadContent($url_info['host'], $url_info['port'], $url_info['request'], $save_file, $content_length, $exists_length, $speed, $headers, $timeout);
            if ($download_status) {
                @unlink($data_file);
            }
        } catch (Exception $e) {
     ...
     * @param $timeout
     */
    public static function downloadContent($host, $port, $url_path, $save_file, $content_length, $range_start, $speed, &$headers, $timeout)
    {
     ...
     * @return int
     */
    public static function getContentSize($host, $port, $url_path, &$headers, $timeout)
    {
        ...
        while (!feof($fsocket)) {
            ...
            if (substr($line, 0, 9) == 'location:') {
                ...
                fclose($fsocket);
                return self::getContentSize($url_info['host'], $url_info['port'], $url_info['request'], $headers, $timeout);
            }
            // get content length
     ...
     * @return string
     */
    public static function buildHeader($action, $url_path, &$headers, $range_start = -1)
    {
